...
private _query = switch (_state) do {
	case 0: { format["`%1licenses`='%2'", getText(_faction >> "DatabaseInfo" >> "queryPrefix"), [_data] call ULP_DB_fnc_mresArray] };
	case 1: {
	...
	case 3: { format["`professions`='%1'", [_data] call ULP_DB_fnc_mresArray] };
	...
	case 6: { format["`achievements`='%1'", [_data] call ULP_DB_fnc_mresArray] };
	case 7: { format["`%1arrested` = '%2', `%1prison_timer` = '%3'", getText(_faction >> "DatabaseInfo" >> "queryPrefix"), [(_data > 0)] call ULP_fnc_bool, [_data, ""] call ULP_fnc_numberText] };
	case 8: { format["`%1daily_tasks`='%2', `%1weekly_tasks`='%3'", getText(_faction >> "DatabaseInfo" >> "queryPrefix"), [_data select 0] call ULP_DB_fnc_mresArray, [_data select 1] call ULP_DB_fnc_mresArray] };
	case 9: { format["`textures`='%1'", [_data] call ULP_DB_fnc_mresArray] };
	case 10: { format["`titles`='%1'", [_data] call ULP_DB_fnc_mresArray] };
	case 11: {
		[_player, "Perks", _data] call ULP_SRV_fnc_setSessionField;
		format["`%1perks`='%2'", getText(_faction >> "DatabaseInfo" >> "queryPrefix"), [_data] call ULP_DB_fnc_mresArray]
	};
	...
	case 14: { format["`blueprints` ='%1'", [_data] call ULP_DB_fnc_mresArray] };
	case 15: {
		[_player, "Gear", _data] call ULP_SRV_fnc_setSessionField;
		format["`%1gear`='%2'", getText(_faction >> "DatabaseInfo" >> "queryPrefix"), [_data] call ULP_DB_fnc_mresArray]
	};
	case 16: { format["`%1stats`='%2'", getText(_faction >> "DatabaseInfo" >> "queryPrefix"), [_data] call ULP_DB_fnc_mresArray] };
...
[format ["UPDATE `players` SET %1 WHERE `pid`='%2'", _query, _steamid], 1] call ULP_DB_fnc_asyncCall;