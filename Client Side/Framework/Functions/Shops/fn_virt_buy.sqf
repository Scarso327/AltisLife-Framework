...
if ([true,_type,_amount] call life_fnc_handleInv) then {
    if (!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") >= _price}) then {
        ...
        if (_action) then {
            hint format ["You bought %1 %2 for £%3 with the group's funds",_amount,(localize _name),[(_price * _amount)] call life_fnc_numberText];
            ...
            hint format ["You bought %1 %2 for £%3",_amount,(localize _name),[(_price * _amount)] call life_fnc_numberText];
