...
if (ULP_SRV_Setting_NextSeason isEqualTo _date) then {
	...
	[format ["UPDATE settings SET value = '%1' WHERE setting = 'Season'", [_season, ""] call ULP_fnc_numberText], 1] call ULP_DB_fnc_asyncCall;
	// Honestly wtf was I doing with this query? It's disgusting but I know it works and lazy mood rn
	[str parseText format ["UPDATE settings SET value = DATE_FORMAT(ADDDATE(CURDATE(), INTERVAL %1 DAY), '&#37;Y-&#37;c-&#37;e') WHERE setting = 'NextSeason'", 14], 1] call ULP_DB_fnc_asyncCall;
};
