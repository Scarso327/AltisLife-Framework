...
if !(_steamid isEqualTo "") then {
	...
	private _query = [format ["SELECT `community_goals`.`id`, `community_goal_contributions`.`contribution` FROM `community_goals` INNER JOIN `community_goal_contributions` ON `community_goals`.`id` = `community_goal_contributions`.`goalId` WHERE `community_goal_contributions`.`pid` = '%1'", _steamid], 2, true] call ULP_DB_fnc_asyncCall;
