...
scopeName "fn_setRestrainedState";

if (canSuspend) exitWith {
    [ULP_fnc_setRestrainedState, _this] call ULP_fnc_directCall;
};
...
// Checks...
if (isNull _target  || { !local _target } || { !(isPlayer _target) } || { !(isNull _detainer) && { !([_detainer] call ULP_fnc_onDuty) } && { (_target distance _detainer) > 5 } }) exitWith { false };
