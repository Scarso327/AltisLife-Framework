...
compileFinal "
    ...
    if (player isEqualTo _unit && (group player) == _group) then {
        ...
        hint ""You have been removed from the group."";
    };
";
...
private [""_msg"",""_to""];
    ...
    hint format [""You have sent a message to all TAS Units."",_to,_msg];
    ctrlShow[3022,true];
";
//To One Person
...
compileFinal "
    ...
    if ((call life_adminlevel) < 1) exitWith {hint ""You are not an staff!"";};
    private [""_msg"",""_to""];
    ctrlShow[3020,false];
    _msg = ctrlText 3003;
    _to = call compile format [""%1"",(lbData[3004,(lbCurSel 3004)])];
