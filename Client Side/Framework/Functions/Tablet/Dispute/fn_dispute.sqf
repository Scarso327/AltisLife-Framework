...
if (time < (missionNamespace getVariable ["ULP_LastDispute", 0])) exitWith {
	["ou've disputed someone recently, please wait..."] call ULP_fnc_hint;
...
[ (findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), ["Police", "Medic", "Civilian"], [_display], {
	...
	[missionConfigFile >> "CfgMessages" >> "Dispute", format ["%1 would like for you to come onto teamspeak (ascensionroleplay.co.uk) to discuss a situation...", name player], _player] call ULP_fnc_sendMessage;
	[format ["You have disputed %1.", name _player]] call ULP_fnc_hint;
