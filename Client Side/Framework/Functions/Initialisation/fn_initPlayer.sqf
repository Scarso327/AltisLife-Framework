...
if (_addActions) then {
	...
	ULP_PlayerActions pushBackUnique (player addAction ["<img image='\a3\ui_f\data\IGUI\Cfg\simpleTasks\types\heal_ca.paa'/> Treat Yourself", { player call ULP_fnc_healAction }, nil, 5, true, true, "", "!([] call ULP_UI_fnc_isProgress) && { [player] call ULP_fnc_canHeal }"]);
	ULP_PlayerActions pushBackUnique (player addAction ["<img image='\a3\ui_f\data\IGUI\Cfg\simpleTasks\types\heal_ca.paa'/> Treat Person", { cursorObject call ULP_fnc_healAction }, nil, 5, true, true, "", "!([] call ULP_UI_fnc_isProgress) && { [cursorObject] call ULP_fnc_canHeal }"]);
