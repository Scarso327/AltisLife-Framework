...
{
	private _query = [format ["SELECT `groups`.`id`, `groups`.`bank`, `groups`.`owner`, `base_bids`.`bid`, `groups`.`tag` FROM `base_bids` INNER JOIN `groups` WHERE `base_bids`.`base` = '%1' AND `base_bids`.`group_id` = `groups`.id AND `base_bids`.`bid` <= `groups`.`bank` AND `base_bids`.`active` = '1' ORDER BY `base_bids`.`bid` DESC", configName _x], 2] call ULP_DB_fnc_asyncCall;
	_query params [
		["_id", -1, [0]], 
		["_bank", 0, [0]], 
		["_owner", "", [""]], 
	...
	[format ["UPDATE settings SET `value` = '%2' WHERE setting = 'base_owner_%1'", configName _x, _id], 1] call ULP_DB_fnc_asyncCall;
	[format ["UPDATE base_bids SET `active` = '0' WHERE base = '%1'", configName _x], 1] call ULP_DB_fnc_asyncCall;
