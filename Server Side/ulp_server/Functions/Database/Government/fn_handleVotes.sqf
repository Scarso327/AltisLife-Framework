...
if !(_winner isEqualTo []) then {
	...
	[_steamid, "Election", ["Winner", [_name, _steamid, [_votes, ""] call ULP_fnc_numberText]]] call ULP_SRV_fnc_logPlayerEvent;

	[format ["UPDATE `settings` SET `value` = '%1' WHERE `setting` = 'Governor'", [[_name, _steamid]] call ULP_DB_fnc_mresArray], 1] call ULP_DB_fnc_asyncCall;

	missionNamespace setVariable ["ULP_SRV_Setting_Governor", [_name, _steamid], true];
};

// Wipe...
["UPDATE `candidates` SET `active` = '0'", 1] call ULP_DB_fnc_asyncCall;
["UPDATE `votes` SET `active` = '0'", 1] call ULP_DB_fnc_asyncCall;