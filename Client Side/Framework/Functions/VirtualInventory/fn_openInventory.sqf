...
if (!(_force) && { (_type in ["Car", "Helicopter", "Plane", "Ship"] && !(_container in ULP_Keys) ) 
	...
	|| { [_container] call ULP_fnc_isHouseStorage && { _container getVariable ["locked", false] } && { !([_container getVariable ["building", objNull]] call ULP_fnc_isHouseOwner) } }
	|| { [_container] call ULP_fnc_isTerritoryContainer && { !([_container getVariable ["Territory", objNull]] call ULP_fnc_ownsSite) } } }) exitWith {
