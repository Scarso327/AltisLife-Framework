...
private _id = -1;
_tag = [_tag] call ULP_DB_fnc_mresString;
_name = [_name] call ULP_DB_fnc_mresString;
...
if !(_query isEqualTo "" || { _query isEqualTo [] }) then {
	...
	[format["UPDATE `groups` SET `owner` = '%1', `type` = '%9', `tag` = '%2', `name` = '%3', `ranks` = '%5', `deposit` = '%6', `withdraw` = '%7', `rank` = '%8', `invite` = '%10', `kick` = '%11', `level` = '1', `xp` = '0', `bank` = '0', `premium` = '0', `active` = '1', `buffs` = '""[]""' WHERE `id` = '%4'",
		_steamid, _tag, _name, _queryId, [_ranks] call ULP_DB_fnc_mresArray, _depositIndex, _withdrawIndex, _rankIndex, _type, _inviteIndex, _kickIndex
	...
	[format[
		...
		[_id, ""] call ULP_fnc_numberText, _steamid, _tag, _name, [_ranks] call ULP_DB_fnc_mresArray, _depositIndex, _withdrawIndex, _rankIndex, _type, _inviteIndex, _kickIndex
	], 1] call ULP_DB_fnc_asyncCall;
