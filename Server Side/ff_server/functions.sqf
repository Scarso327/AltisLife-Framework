...
private [""_msg"",""_to""];
    ...
    if (_length > 400) exitWith {hint localize ""Your message cannot exceed 400 characters."";ctrlShow[3022,true];};
    _to = ""EMS Units"";
...
compileFinal "
    ...
    _length = count (toArray(_msg));
    if (_length > 400) exitWith {hint localize ""Your message cannot exceed 400 characters."";ctrlShow[3015,true];};
    if (lbCurSel 3004 isEqualTo -1) exitWith {hint localize ""You must select a person to message.""; ctrlShow[3015,true];};

    _to = call compile format [""%1"",(lbData[3004,(lbCurSel 3004)])];
    if (isNull _to) exitWith {ctrlShow[3015,true];};
    if (isNil ""_to"") exitWith {ctrlShow[3015,true];};
    if (_msg isEqualTo """") exitWith {hint localize ""You must enter a message to send."";ctrlShow[3015,true];};

    [_msg,name player,0] remoteExecCall [""TON_fnc_clientMessage"",_to];
    [] call life_fnc_cellphone;
    hint format [localize ""You sent %1 a message: %2"",name _to,_msg];
    ctrlShow[3015,true];
";
//To All Cops
...
compileFinal "
    ...
    if (_msg isEqualTo """") exitWith {hint localize ""You must enter a message to send."";ctrlShow[3016,true];};
    _length = count (toArray(_msg));
    if (_length > 400) exitWith {hint localize ""Your message cannot exceed 400 characters."";ctrlShow[3016,true];};

    [_msg,name player,1,mapGridPosition player,player] remoteExecCall [""TON_fnc_clientMessage"",-2];
    [] call life_fnc_cellphone;
    hint format [localize ""You sent %1 a message: %2"",_to,_msg];
    ctrlShow[3016,true];
";
//To All Admins
...
compileFinal "
    ...
    if (_msg isEqualTo """") exitWith {hint localize ""You must enter a message to send."";ctrlShow[3017,true];};
    _length = count (toArray(_msg));
    if (_length > 400) exitWith {hint localize ""Your message cannot exceed 400 characters."";ctrlShow[3017,true];};
...
compileFinal "
    ...
    if ((call life_adminlevel) < 1) exitWith {hint localize ""You are not a member of staff."";};
    private [""_msg"",""_to""];
    ctrlShow[3020,false];
    _msg = ctrlText 3003;
    _to = call compile format [""%1"",(lbData[3004,(lbCurSel 3004)])];
    ...
    if (_msg isEqualTo """") exitWith {hint localize ""You must enter a message to send."";ctrlShow[3020,true];};

    [_msg,name player,3] remoteExecCall [""TON_fnc_clientMessage"",_to];
    [] call life_fnc_cellphone;
    hint format [localize ""Staff message sent to: %1: %2"",name _to,_msg];
    ctrlShow[3020,true];
";
...
compileFinal "
    ...
    ctrlShow[3021,false];
    _msg = ctrlText 3003;
    if (_msg isEqualTo """") exitWith {hint localize ""You must enter a message to send."";ctrlShow[3021,true];};

    [_msg,name player,4] remoteExecCall [""TON_fnc_clientMessage"",-2];
    [] call life_fnc_cellphone;
    hint format [localize ""Staff message sent to everyone: %1"",_msg];
    ctrlShow[3021,true];
";
