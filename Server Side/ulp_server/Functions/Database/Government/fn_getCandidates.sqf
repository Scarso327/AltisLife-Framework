...
private _query = ["SELECT `candidates`.`id`, `players`.`pid`, `candidates`.`name`, `groups`.`name`, `groups`.`type`, `groups`.`level` FROM `candidates` INNER JOIN `players` ON `players`.`pid` = `candidates`.`pid` INNER JOIN `groups` ON `groups`.`id` = `players`.`group_id` WHERE `candidates`.`active` = '1' AND `groups`.`active` = '1'", 2, true] call ULP_DB_fnc_asyncCall;
