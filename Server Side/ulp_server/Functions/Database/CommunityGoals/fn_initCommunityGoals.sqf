...
private _query = ["SELECT community_goals.id, community_goals.goal_cfg, DATEDIFF(community_goals.end_date, CURDATE()) as daysLeft, COALESCE((SELECT SUM(community_goal_contributions.contribution) FROM community_goal_contributions WHERE community_goal_contributions.goalId = community_goals.id), 0) AS progress FROM community_goals WHERE community_goals.paid = '0'", 2, true] call ULP_DB_fnc_asyncCall;
...
["CALL deleteOldCommunityGoals", 1] call ULP_DB_fnc_asyncCall;
