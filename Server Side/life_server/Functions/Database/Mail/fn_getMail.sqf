...
private _query = [format["SELECT `id`, `pid`, `type`, `class`, `data`, `daysUntilExpiry` FROM `mail` WHERE `pid`='%1' AND `claimed`='0' AND (`daysUntilExpiry` IS NULL OR `daysUntilExpiry` > 0)", _pid], 2, true] call DB_fnc_asyncCall;
