...
if !(([format["SELECT * FROM `candidates` WHERE `pid` = '%1'", _steamid], 2] call ULP_DB_fnc_asyncCall) isEqualTo []) exitWith {
...
[format["INSERT INTO `candidates` (`pid`, `name`) VALUES ('%1', '%2');", _steamid, [name _candidate] call ULP_DB_fnc_mresString], 1] call ULP_DB_fnc_asyncCall;
