...
if (_sellPrice > 0) then {
	if (_configName in (missionNamespace getVariable ["ULP_SRV_Setting_BuffedItem", []])) then { _sellPrice = _sellPrice * getNumber(missionConfigFile >> "CfgSettings" >> "buffedSellIncrease"); };
