...
CONTROL(2600,2601) ctrlSetStructuredText parseText format ["<t align='center'><t size='.8px'>" +(localize "%1 has given you a ticket for £%2."),_cop getVariable ["realname",name _cop],_val];

[] spawn {
    disableSerialization;
    waitUntil {life_ticket_paid || (isNull (findDisplay 2600))};
    if (isNull (findDisplay 2600) && !life_ticket_paid) then {
        [0,"%1 refused to pay the ticket.",true,[profileName]] remoteExecCall ["life_fnc_broadcast",west];
        [1,"%1 refused to pay the ticket.",true,[profileName]] remoteExecCall ["life_fnc_broadcast",life_ticket_cop];
    };
};