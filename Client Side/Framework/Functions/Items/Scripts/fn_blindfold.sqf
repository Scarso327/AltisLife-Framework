...
if (_unit getVariable ["blindfold", false]) then {
	...
	[format ["You have removed %1's blindfold...", [_unit] call ULP_fnc_getName]] call ULP_fnc_hint;
} else {
	if (["Blindfold", 1, true] call ULP_fnc_handleItem) then {
		_unit setVariable ["blindfold", true, true];
		[format ["You have blindfolded %1...", [_unit] call ULP_fnc_getName]] call ULP_fnc_hint;
	} else {
