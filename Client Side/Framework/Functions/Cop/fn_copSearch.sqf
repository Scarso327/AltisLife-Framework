...
if (count _invs > 0) then {
    ...
    [0,"%1 has £%2 worth of contraband on them.",true,[(_civ getVariable ["realname",name _civ]),[_illegal] call life_fnc_numberText]] remoteExecCall ["life_fnc_broadcast",west];
} else {
...
if (_robber) then {
    [0,"%1 was identified as the bank robber!",true,[(_civ getVariable ["realname",name _civ])]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
};
