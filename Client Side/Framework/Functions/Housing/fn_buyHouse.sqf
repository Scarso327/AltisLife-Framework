...
if (_house getVariable ["house_owned",false]) exitWith {hint "This property is already owned...";};
...
if (!license_civ_home) exitWith {hint "You do not have the required license!"};
if (count life_houses >= (LIFE_SETTINGS(getNumber,"house_limit"))) exitWith {hint format ["You can only own %1 properties at a time.",LIFE_SETTINGS(getNumber,"house_limit")]};
closeDialog 0;
...
_action = [
    ...
    (_houseCfg select 1)],"Purchase Property","Buy","Cancel"
