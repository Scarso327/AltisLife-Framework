...
(CONTROL(2620,601)) ctrlSetText format [("Funds:")+ " $%1",[_gangBank] call life_fnc_numberText];
...
lbClear _members;
{
    if ((getPlayerUID _x) == _ownerID) then {
        _members lbAdd format ["%1 " +(localize "(Group Leader)"),(_x getVariable ["realname",name _x])];
