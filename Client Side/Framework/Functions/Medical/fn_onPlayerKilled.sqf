...
_unit spawn {
    ...
    waitUntil {
        _Timer ctrlSetText format ["Respawn Available in: %1",[(_maxTime - time),"MM:SS"] call BIS_fnc_secondsToString];
...
if (side _killer isEqualTo west && !(playerSide isEqualTo west)) then {
    ...
    if (!life_use_atm && {CASH > 0}) then {
        [format ["£%1 from the National Treasury robbery was returned from the robber being killed.",[CASH] call life_fnc_numberText]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
        CASH = 0;
    };
};
