...
switch (_menu) do {
	case "General": {
		...
		// Setup buttons...
		if (isDowned(life_pInact_curTarget) && (((independent countSide playableUnits) <= 0 && { (playerSide isEqualTo west && {(LIFE_SETTINGS(getNumber,"revive_cops") isEqualTo 1)}) }) || { playerSide isEqualTo independent })) then {
			_buttons pushBack ["Revive Player", "life_pInact_curTarget spawn FF_fnc_revive"]; // Revive Button...

			// Administer Blood Button...
			if (ITEM_VALUE("blood_bag") > 0) then {
				_buttons pushBack ["Administer Blood", "life_pInact_curTarget spawn FF_fnc_administerBlood"];
			};
		} else {
	...
	case "Police": {
		...
		if (player getVariable ["isEscorting",false]) then {
			...
			_buttons pushBack ["Seize Contraband", "[life_pInact_curTarget] spawn life_fnc_seizePlayerAction; closeDialog 0;"] // Seize Contraband Button...
		};
	};
