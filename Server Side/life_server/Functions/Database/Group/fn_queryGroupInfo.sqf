...
if (isNull _group) then {
	private _query = [format["SELECT groups.id, groups.owner, players.group_level, groups.name, groups.ranks, groups.bank, groups.premium, groups.deposit, groups.withdraw, groups.rank FROM groups INNER JOIN players ON players.group_id = groups.id AND players.pid = '%1' WHERE (groups.owner = '%1' OR groups.id = '%2') AND groups.active='1'",
		_steamid, _groupid
	], 2] call DB_fnc_asyncCall;

	if !(_query isEqualTo "" || { _query isEqualTo [] }) exitWith {
		_query params [
			"_queryId", "_queryOwner", "_queryRank", "_queryName", "_queryRanks", "_queryBank", "_queryPremium", "_queryDeposit", "_queryWithdraw", "_queryRankPerm"
		];
		...
		_group setVariable ["group_permissions", [_queryDeposit, _queryWithdraw, _queryRankPerm], true];
