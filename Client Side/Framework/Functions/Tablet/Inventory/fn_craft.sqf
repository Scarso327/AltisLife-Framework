...
if !(isNull (objectParent player)) exitWith {
	["You can't craft items while in a vehicle..."] call ULP_fnc_hint;
	false;
};
...
if ([getNumber (_cfg >> "workbench")] call ULP_fnc_bool && { !([] call ULP_fnc_nearWorkbench) }) exitWith {
	["This blueprint requires a workbench to construct..."] call ULP_fnc_hint;
	false
};
...
if (_total < 1) exitWith {
	["You don't have all the materials required to craft this blueprint..."] call ULP_fnc_hint;
	false
};
...
[
	(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), [1, _total], [_cfg, _name],
	{
		...
		[format["Constructing %1(s)", _name], getNumber (_cfg >> "time") * _amount, [_cfg, _name, _amount], { true }, {
