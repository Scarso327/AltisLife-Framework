...
if (([] call ULP_SRV_fnc_getDayName) isEqualTo getText (missionConfigFile >> "CfgBases" >> "Day") && { _hour >= _startHour } && { _hour < _endHour }) then {
	if !(ULP_SRV_Setting_BaseBidsActive) then {
		["UPDATE settings SET `value` = '1' WHERE setting = 'BaseBidsActive'", 1] call ULP_DB_fnc_asyncCall;
	...
	if (ULP_SRV_Setting_BaseBidsActive) then {
		["UPDATE settings SET `value` = '0' WHERE setting = 'BaseBidsActive'", 1] call ULP_DB_fnc_asyncCall;
...

{
	...
	private _query = [format ["SELECT `settings`.`value`, `groups`.`tag` FROM `settings` 
		...
		WHERE `settings`.`setting` = 'base_owner_%1'", configName _x], 2] call ULP_DB_fnc_asyncCall;
	_query params [
		["_owner", -2, [0, ""]],
		["_groupTag", "", [""]]
	];
	...
	if (_owner isEqualTo -2) then {
		// Insert...
		[format ["INSERT INTO settings (setting, `value`) VALUES ('base_owner_%1', '%2');", configName _x, getNumber(_x >> "defaultGroupOwnerId")], 1] call ULP_DB_fnc_asyncCall;
	} else {
		...
		if ([getNumber (_x >> "includeBidding")] call ULP_fnc_bool && { missionNamespace getVariable ["ULP_SRV_Setting_BaseBidsActive", false] }) then {
			_owner = -1;
			[format ["UPDATE settings SET `value` = '-1' WHERE setting = 'base_owner_%1'", configName _x], 1] call ULP_DB_fnc_asyncCall;
		};
	};
