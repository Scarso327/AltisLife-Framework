#define UI_WIDTH 0.35
#define UI_X 0.5 - (UI_WIDTH / 2)
#define BODY_HEIGHT 0.450
#define BODY_Y 0.5 - (BODY_HEIGHT / 2)

#define BLUEPRINT_LIST_W (UI_WIDTH * 0.4) - (MARGIN_X * 2)
#define MAIN_H BODY_HEIGHT - (0.022 * 2) - (MARGIN_Y * 2)

class DialogVehicleFactory {
    idd = 6200;
    movingEnable = 0;
    enableSimulation = 0;
    onInit = "_this call ULP_UI_fnc_DialogVehicleFactory_OnInit";
    onLoad = "_this call ULP_UI_fnc_DialogVehicleFactory_OnLoad";
	onUnload = "_this call ULP_UI_fnc_DialogVehicleFactory_OnUnload";

    class ControlsBackground {
		class Header : Life_RscStructuredText {
			idc = 6201;
			colorBackground[] = HEADER_COLOUR;
			text = "%1 Factory";
			SAFEZONE_X(UI_X);
			SAFEZONE_Y(BODY_Y - 0.022);
			SAFEZONE_W(UI_WIDTH);
			SAFEZONE_H(0.022);
		};

		class Body : Life_RscText {
			idc = -1;
			colorBackground[] = BODY_COLOUR;
			SAFEZONE_X(UI_X);
			SAFEZONE_Y(BODY_Y);
			SAFEZONE_W(UI_WIDTH);
			SAFEZONE_H(BODY_HEIGHT);
		};

		class Footer : Life_RscText {
			idc = -1;
			colorBackground[] = FOOTER_COLOUR;
			SAFEZONE_X(UI_X);
			SAFEZONE_Y((BODY_Y + BODY_HEIGHT));
			SAFEZONE_W(UI_WIDTH);
			SAFEZONE_H(0.033);
		};
	};

	class Controls {
		class BlueprintList : Life_RscListBox {
			idc = 6202;
			colorBackground[] = INNER_BODY_COLOUR;
			onLBSelChanged = "_this call ULP_UI_fnc_DialogVehicleFactory_OnListChanged;";
			rowHeight = 0.04;
			sizeEx = 0.038;
			SAFEZONE_X(UI_X + MARGIN_X);
			SAFEZONE_Y(BODY_Y + MARGIN_Y);
			SAFEZONE_W(BLUEPRINT_LIST_W);
			SAFEZONE_H(BODY_HEIGHT - (MARGIN_Y * 2));
		};

		class ItemsView : Life_RscControlsGroup {
			idc = 6203;
			SAFEZONE_X(UI_X + (MARGIN_X * 2) + BLUEPRINT_LIST_W);
			SAFEZONE_Y(BODY_Y + MARGIN_Y);
			SAFEZONE_W(UI_WIDTH + MARGIN_X - BLUEPRINT_LIST_W);
			SAFEZONE_H(MAIN_H);

			class Controls {
				class ItemInformation : Life_RscStructuredText {
					idc = 6205;
					text = "";
					colorBackground[] = INNER_BODY_COLOUR;
					y = 0;
					x = 0;
					SAFEZONE_W(UI_WIDTH + MARGIN_X - BLUEPRINT_LIST_W);
					SAFEZONE_H(MAIN_H);
				};
			};
		};

		class Background: Life_RscText {
			idc = -1;
			colorBackground[] = {0.1,0.1,0.1,0.85};
			SAFEZONE_X(UI_X + (MARGIN_X * 2) + BLUEPRINT_LIST_W);
			SAFEZONE_Y(BODY_Y + MAIN_H + 0.022 + MARGIN_Y);
			SAFEZONE_W(UI_WIDTH + MARGIN_X - BLUEPRINT_LIST_W);
			SAFEZONE_H(0.022);
		};
	
		class ProgressBar: Life_RscProgress {
			idc = 6206;
			SAFEZONE_X(UI_X + (MARGIN_X * 2) + BLUEPRINT_LIST_W);
			SAFEZONE_Y(BODY_Y + MAIN_H + 0.022 + MARGIN_Y);
			SAFEZONE_W(UI_WIDTH + MARGIN_X - BLUEPRINT_LIST_W);
			SAFEZONE_H(0.022);
		};

		class ProgressText: Life_RscStructuredText {
			idc = 6207;
			text = "No current order...";
			SAFEZONE_X(UI_X + (MARGIN_X * 2) + BLUEPRINT_LIST_W);
			SAFEZONE_Y(BODY_Y + MAIN_H + MARGIN_Y);
			SAFEZONE_W(UI_WIDTH + MARGIN_X - BLUEPRINT_LIST_W);
			SAFEZONE_H(0.022);
			class Attributes
			{
				align = "center";
			};
		};
		
		class CraftButton : Life_RscButtonCenter {
			idc = 6204;
			text = "<t align = 'center'>Place Order</t>";
			onButtonClick = "_this call ULP_UI_fnc_DialogVehicleFactory_PlaceOrder;";
			SAFEZONE_X((UI_X + UI_WIDTH - (UI_WIDTH / 4)) - MARGIN_X);
			SAFEZONE_Y((BODY_Y + BODY_HEIGHT) + BUTTON_MARGIN_Y);
			SAFEZONE_W((UI_WIDTH / 4));
			SAFEZONE_H(0.022);
		};
	};
};