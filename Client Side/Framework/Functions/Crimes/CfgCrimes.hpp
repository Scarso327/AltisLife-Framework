		...
		class HMTreasury {
			...
			class Stages {
				class Hack {
					...
					onEachFrame = "";
					onProgressStop = "";
					onFail = "[(switch (false) do { case ((count ([""Police""] call ULP_fnc_allMembers)) >= 12) : { ""There must be at least <t color='#B92DE0'>12 police officers</t> to start this crime..."" }; case ([""HackingDevice""] call ULP_fnc_hasItem < 1) : { ""You need a hacking device to perform this action..."" }; default { ""Vault Security has already been disabled..."" }; })] call ULP_fnc_hint;";
					condition = "!(missionNamespace getVariable [""ULP_SRV_Crime_HMTreasury"", false]) && { [""HackingDevice""] call ULP_fnc_hasItem > 0 } && { (count ([""Police""] call ULP_fnc_allMembers)) >= 12 }";
				};
			};
			class Buildings {
				class MilitaryOfficeOne {
					...
					class BreakIn {
						...
						onFail = "[""There must be at least <t color='#B92DE0'>12 police officers</t> to start this crime...""] call ULP_fnc_hint;";
						condition = "(count ([""Police""] call ULP_fnc_allMembers)) >= 12 || { missionNamespace getVariable [""ULP_SRV_Crime_HMTreasury"", false] }";
					};
				};
		...
		class HMSLiberty {
			...
			class Stages {
				class Hack {
					...
					onFail = "[(switch (false) do { case ((count ([""Police""] call ULP_fnc_allMembers)) >= 12) : { ""There must be at least <t color='#B92DE0'>12 police officers</t> to start this crime..."" }; case ([""HackingDevice""] call ULP_fnc_hasItem < 1) : { ""You need a hacking device to perform this action..."" }; default { ""Server Security has already been disabled..."" }; })] call ULP_fnc_hint;";
			...
			class Items {
				class EncryptedDrive {
					amount = 1;
					extra = "((count playableUnits) / 6) + ((time / 10) / 240)";
				};
			};
		};
		class PoliceEvidenceStorage {
			...
			class Stages {
				class Drill {
					...
					onEachFrame = "";
					onProgressStop = "";
					onFail = "[(switch (false) do { case ((count ([""Police""] call ULP_fnc_allMembers)) >= 8) : { ""There must be at least <t color='#B92DE0'>8 police officers</t> to start this crime..."" }; case ([""IndustrialDrill""] call ULP_fnc_hasItem < 1) : { ""You need an industrial drill to perform this action..."" }; default { ""Valut security has already been disabled..."" }; })] call ULP_fnc_hint;";
			...
			class Buildings {
				class MilitaryOffice {
					...
					class BreakIn {
						...
						onFail = "[""There must be at least 8 police officers to start this crime...""] call ULP_fnc_hint;";
		...
		class CasinoVault {
			...
			class Stages {
				class Drill {
					...
					onProgressStop = "";
					onFail = "[(switch (false) do { case ((count ([""Police""] call ULP_fnc_allMembers)) >= 8) : { ""There must be at least <t color='#B92DE0'>8 police officers</t> to start this crime..."" }; case ([""IndustrialDrill""] call ULP_fnc_hasItem < 1) : { ""You need an industrial drill to perform this action..."" }; default { ""Valut security has already been disabled..."" }; })] call ULP_fnc_hint;";
					condition = "!(missionNamespace getVariable [""ULP_SRV_Crime_CasinoVault"", false]) && { [""IndustrialDrill""] call ULP_fnc_hasItem > 0 }";
				};
			};
			class Buildings {
				class MilitaryOffice {
					classname = "Land_Hotel_02_F";
					position[] = {7917.68, 16155.3, -0.0833969};
					class BreakIn {
						breakIn = 600;
						onBreakIn = "if (([""SilentLocksmith"", (random 1)] call ULP_fnc_activatePerk) < 0.8) exitWith {}; private _cfg = missionConfigFile >> ""CfgMessages"" >> ""BreakIn""; private _targets = getText (_cfg >> ""targets""); [_cfg, format [""A door at the Police Evidence Storage facility has been broken into!""], _targets] call ULP_fnc_sendMessage; [""You accidently tripped an alarm while breaking the lock, police have been alerted!""] call ULP_fnc_hint;";
						onFail = "[""There must be at least 10 police officers to start this crime...""] call ULP_fnc_hint;";
						condition = "(count ([""Police""] call ULP_fnc_allMembers)) >= 8 || { missionNamespace getVariable [""ULP_SRV_Crime_CasinoVault"", false] }";
					};
				};
			};
			class Items {
