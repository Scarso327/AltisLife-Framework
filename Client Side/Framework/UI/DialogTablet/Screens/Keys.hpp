		...
		class ShareOption : Life_RscControlsGroupNoScrollbars {
			...
			class Controls {
				...
				class OptionButton : ULP_RscButtonClean {
					...
					onButtonClick = "private _house = (_this select 0) getVariable [""house"", objNull]; if ([_house] call ULP_fnc_shareHouse) then { (_this select 0) ctrlSetStructuredText parseText format[""<t align = 'center'>%1</t>"", [""Share"",""Unshare""] select ([_house] call ULP_fnc_isHouseShared)] };";
