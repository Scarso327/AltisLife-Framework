...
private _query = [format["SELECT count(`id`) FROM `vehicles` WHERE `pid`='%1' AND `classname`='%2' AND `faction`='%3' AND `sold`='0'", _steamId, _className, _faction], 2] call ULP_DB_fnc_asyncCall;
