...

{
	...
	if (isClass _houseCfg) then {
		// Convert Datatypes...
		_pos = [_pos] call ULP_DB_fnc_mresToArray;
		_storage = [_storage] call ULP_DB_fnc_mresToArray;
		_virtualStorage = [_virtualStorage] call ULP_DB_fnc_mresToArray;
		_upgrades = createHashMapFromArray ([_upgrades] call ULP_DB_fnc_mresToArray);
		...
		if (isNull _house || { _house in ULP_SRV_Houses }) then {
			...
			switch (true) do {
				case (_house getVariable ["blacklisted", false] || { ["redzone_", [_house]] call ULP_fnc_isUnitsInZone }): {
					...
					[format["UPDATE `houses` SET `sold`='1' WHERE `id`='%1'", [_id, ""] call ULP_fnc_numberText], 1] call ULP_DB_fnc_asyncCall;
