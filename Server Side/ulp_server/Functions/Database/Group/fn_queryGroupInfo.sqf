...
if (isNull _group) then {
	...
	if !(_query isEqualTo "" || { _query isEqualTo [] }) exitWith {
		...
		_queryRanks = [_queryRanks] call ULP_DB_fnc_mresToArray;
		...
		_group setVariable ["group_permissions", createHashMapFromArray [
			["deposit", _queryDeposit], 
			["withdraw", _queryWithdraw], 
			["rank", _queryRankPerm], 
			["invite", _queryInvite], 
			["kick", _queryKick]], true];
		...
		private _members = [format ["SELECT `name`, `pid`, `group_level` FROM `players` WHERE `group_id` = '%1'", _groupid], 2, true] call ULP_DB_fnc_asyncCall;
		...
		private _alliances = [format ["SELECT `group_a_id`, `groupa`.`name`, `group_b_id`, `groupb`.`name` FROM `group_alliances` INNER JOIN `groups` `groupa` ON `groupa`.`id` = `group_alliances`.`group_a_id` INNER JOIN `groups` `groupb` ON `groupb`.`id` = `group_alliances`.`group_b_id` WHERE `group_a_id` = '%1' OR `group_b_id` = '%1'", _groupid], 2, true] call ULP_DB_fnc_asyncCall;
		...
		_queryBuffs = [_queryBuffs] call ULP_DB_fnc_mresToArray;
